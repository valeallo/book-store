...
const Navbar = () => {
  ...
  const filterBooks = (array, title) => {
    ...
    return array.filter((book) => {
      if (title === "") {
        return;
      } else if (book.title.toLowerCase().includes(title.toLowerCase()))
    ...
    <div className="relative  w-full">
      <div className="flex items-center justify-between flex-wrap  bg-[#FF577F] p-4">
              ...
              {NavbarLink.map((page, index) => {
                return (
                  <Link key={index} to={page.link}>
                    <li className="block mt-4 lg:inline-block lg:mt-0 text-[#fda9bd] hover:text-white mr-4">
                      {page.title}
                    </li>
                  </Link>
                );
        ...
        <div className="mr-5">
          ...
          >
            <SearchIcon className="flex items-center flex-shrink-0 text-white ml-5" />
          </button>
        </div>
      </div>
      {result.length > 0 && modalSearchTest && (
        <div className="absolute top-12 right-6 w-[350px] bg-zinc-100 z-10 flex flex-col justify-center  p-4">
          <div
            ...
            onClick={() => setModalSearchTest(false)}
          >
            &times;
          </div>
          <ul>
